...
#   format of command is >python CellFile_NetMon.py
...
def mdbGQueryData(DB):
    # getting query of GSM cell info from MicrosoftSQL Server 2014 on NPO UA Server

    DRV = '{SQL Server}'
    SRV = '10.156.48.253'

    # connect to db
    cnxn = pyodbc.connect("Driver={};Server={};Database={};Uid={}".format(DRV, SRV, DB, LOGIN))
...
def mdbWQueryData(DB):
    # getting query of WCDMA cell info from MicrosoftSQL Server 2014 on NPO UA Server in ODBC data admin
...
def mdbLQueryData(DB):

    # getting query of LTE cell info from MicrosoftSQL Server 2014 on NPO UA Server
    DRV = '{SQL Server}'
    SRV = '10.156.48.253'
...
if __name__ == '__main__':
    ...
    cellfile = 'CF_NetMon_' + database + '_' + now.strftime("%Y%m%d") + '.csv'
    path = 'C:\\01_Python_gen\\NetMonitorCellFile\\Output\\'

    print('\n', 'Cooking CellFile for NetMonitor from Atoll SQL DB ' + database + ':\n')

    with open(path + cellfile, 'w') as cf:
    ...
    print("\n{} generated for {} seconds!".format(cellfile, round(time()-t)))
